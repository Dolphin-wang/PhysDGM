...
import math
import wandb
from data_process import windowarray
...
def test_data_load_one(windowsize, fpt, dataset='FD001', engine_number = 24):
    array_rul = pd.read_csv('./data/RUL_'+dataset+'.txt', header=None, sep=' ').drop(columns=[1]).values
    load_name = './dataset/' + dataset + '_np_norm/test_' + dataset + '_norm_'
    dim = np.load(load_name + str(engine_number) + '.npy').shape
    cur_test = torch.randn(1, windowsize, dim[1])
    cur_test_label = torch.randn(1, 1)
    print('reading test data')
    j = 0
    read_name = load_name + str(engine_number) + '.npy'
    data = np.load(read_name)
    if data.shape[0] >= windowsize:
        j += 1
        windowdata, windowlabel = windowarray(data, windowsize, fpt, array_rul[engine_number-1][0])
        cur_test = torch.cat((cur_test, windowdata), 0)
        cur_test_label = torch.cat((cur_test_label, windowlabel), 0)
    print('test samples: ', j)
    cur_test = cur_test[1:]
    cur_test_label = cur_test_label[1:]
    return cur_test, cur_test_label, j