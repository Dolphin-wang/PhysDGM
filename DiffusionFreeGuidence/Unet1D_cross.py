...
from DiffusionFreeGuidence.Attetnion import crossAttention,crossFrequencyAttention, crossFrequencyAttention1
    ...
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                block_klass(dim_in, dim_in, time_emb_dim = time_dim, classes_emb_dim = classes_dim),
                Residual(PreNorm(dim_in, crossAttention(length // 2**(ind)))),
                Downsample(dim_in, dim_out) if not is_last else nn.Conv1d(dim_in, dim_out, 3, padding = 1)
            ]))

        mid_dim = dims[-1]
        self.mid_block1 = block_klass(mid_dim, mid_dim, time_emb_dim = time_dim, classes_emb_dim = classes_dim)
        self.mid_attn = Residual(PreNorm(mid_dim, crossAttention(length // 2**(ind))))
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                block_klass(dim_out + dim_in, dim_out, time_emb_dim = time_dim, classes_emb_dim = classes_dim),
                Residual(PreNorm(dim_out, crossAttention(length // 2**(len(in_out)-ind-1)))),
