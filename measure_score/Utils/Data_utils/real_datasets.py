...
from measure_score.Models.interpretable_diffusion.model_utils import normalize_to_neg_one_to_one, unnormalize_to_zero_to_one
from measure_score.Utils.masking_utils import noise_mask
...
class CustomDataset(Dataset):
    ...
    def __getsamples(self, data, proportion, seed):
        ...
        if self.name == 'fd001':
            train_data = np.load('D:/python/bishe/Diffusion-TS-main/OUTPUT/fd001/train_fd001.npy')
            train_data = normalize_to_neg_one_to_one(train_data)
            #np.save('D:/python/bishe/Diffusion-TS-main/OUTPUT/fd001/train_fd001.npy',train_data)
        #print("train_data.shape",train_data.shape)
        #print("test_data.shape",test_data.shape)
        if self.name == 's02':
            self.var_num = 19
            train_data = np.load('D:/python/bishe/Diffusion-TS-main/Data/train_data.npy')
            test_data = np.load('D:/python/bishe/Diffusion-TS-main/Data/test_data.npy')
            train_data = normalize_to_neg_one_to_one(train_data)
        elif self.name == 'fd001_new':
            self.var_num = 16
            train_data = np.load('D:/python/bishe/data_process/data_new/train_fd001_new.npy')
            train_data = normalize_to_neg_one_to_one(train_data)
        elif self.name == 'fd001and003':
            self.var_num = 16
            train_data = np.load('D:/python/bishe/data_process/data_new/train_fd001and003.npy')
            train_data = normalize_to_neg_one_to_one(train_data)
        '''if self.save2npy and self.name != 's02' and self.name != 'fd001':
            if 1 - proportion > 0 and self.name != 's02':
            ...
            if self.auto_norm and self.name != 's02':
                if 1 - proportion > 0:
    ...
    def read_data_fd001(self):
        """Reads a single .csv
        """
        df = pd.read_csv('./Data/datasets/fd001_data.csv', header=0)
        data = df.values
        return data
