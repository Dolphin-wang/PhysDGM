...
from measure_score.Models.ts2vec.ts2vec import TS2Vec
...
def Context_FID(ori_data, generated_data):
    print("---------context fid!------------")
    model = TS2Vec(input_dims=ori_data.shape[-1], device=0, batch_size=256, lr=0.001, output_dims=320,
    ...
    idx = np.random.permutation(min(ori_data.shape[0],generated_data.shape[0]))
